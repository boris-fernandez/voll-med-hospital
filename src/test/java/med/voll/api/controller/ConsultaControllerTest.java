...
class ConsultaControllerTest {
    ...
    @MockBean
    private ReservarDeConsultas reservarDeConsultas;

    @Test
    @DisplayName("Deberia devolver htto 400 cuando la request no tenga datos")
    @WithMockUser
    ...
    @Test
    @DisplayName("Deberia devolver htto 200 cuando la request reciba un json valido")
    @WithMockUser
    void reservar_escenario2() throws Exception {
        var fecha = LocalDateTime.now().plusHours(1);
        var especialidad = Especialidad.CARDIOLOGIA;
        var datosDetalle = new DatosDetalleConsulta(null, 2L, 5L, fecha);
        when(reservarDeConsultas.reservar(any())).thenReturn(datosDetalle);
